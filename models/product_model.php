<?php 
    ...
    function get_products(){
        // require'pdo.php';
        $sql = "SELECT * FROM `products` INNER JOIN `images` ON `images`.product_id = `products`.product_id INNER JOIN categories 
    ...
    function add_product($arr){
        ...
        $sql = "INSERT INTO `products` (`product_name`, `product_price`, `product_des`, `product_quantity`, `category_id`,`product_img`)  
    ...
    function edit_product($arr,$id){
        ...
        $sql = "UPDATE `products` SET `product_name`='$product_name', `product_price`=$product_price, `product_des`='$product_des', `product_quantity`=$product_quantity, `category_id`=$category_id WHERE `product_id` = {$id};";
        pdo_excute($sql);
